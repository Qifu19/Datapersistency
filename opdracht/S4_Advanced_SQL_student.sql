...
SELECT medewerkers.mnr AS nummer, medewerkers.functie AS functie, medewerkers.gbdatum AS geboortedatum
FROM medewerkers
WHERE medewerkers.gbdatum < DATE '1980-01-01'
AND (medewerkers.functie = 'TRAINER' OR medewerkers.functie = 'VERKOPER');
...
SELECT medewerkers.naam AS naam
FROM medewerkers
WHERE medewerkers.naam LIKE '% %';
...
SELECT inschrijvingen.cursus AS code, inschrijvingen.begindatum , COUNT(inschrijvingen.cursist) AS aantal_inschrijvingen
FROM inschrijvingen
WHERE EXTRACT(YEAR FROM inschrijvingen.begindatum) = 2019
...
SELECT uitvoeringen.cursus AS cursus, COUNT(*) AS aantal
FROM uitvoeringen
GROUP BY cursus;
